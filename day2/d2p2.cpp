...
bool isSafe(std::vector<int> row) {
  for (int i = 0; i < row.size(); ++i) {
    std::vector<int> toleratedRow;
    for (int j = 0; j < row.size(); ++j) {
      if (j != i) {
        toleratedRow.push_back(row[j]);
      }
    }
    // Print the new vector for testing purposes
    std::cout << "New row without element at index " << i << ": ";
    for (int val : toleratedRow) {
      std::cout << val << " ";
    }
    std::cout << std::endl;

    if (isSorted(toleratedRow) && isValidDifference(toleratedRow)) {
      return true;
    }
  }
